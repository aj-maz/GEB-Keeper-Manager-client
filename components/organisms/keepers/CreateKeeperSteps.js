...
const CreateKeeperSteps = ({systems}) => {
    ...
    const generateWalletObject = () => {
        ...
        if (walletSettings.variant === 2) return new ethers.Wallet(walletSettings.fromPrivateKey.privateKey)
    }

    const networksRPCAddresses = {
        mainnet: 'https://eth-mainnet.g.alchemy.com/v2/PKDcpW-zo09u7KieHzUl5H0qujGgr5nv',
        goerli: 'https://goerli.infura.io/v3/7a3ae98793214e75b0686d31b7fa2c56'
    }
    ...
    const getWalletBalances = async (walletObject) => {
        ...
        const selectedNetworkObj = selectedSystemObj?.networks.find((network) => network.name === system.network);

        const systemCoinAddress = selectedNetworkObj?.systemCoin

        console.log(selectedNetworkObj, selectedSystemObj, system.network, system.name, systemCoinAddress)

        const systemCoinContract = new ethers.Contract(systemCoinAddress, ERC20, provider)
    ...
    const steps = [
        {
            ...
            content: <KeeperSystemStep systems={systems} systemSettings={keeperSettings.system}
                                       setSystemSettings={changeKeeperSetting('system')}/>,
        },
        {
            label: 'Keeper Wallet',
            caption: 'Choose the wallet that runs the keeper',
            content: <WalletSelectionStep setWalletSetup={changeKeeperSetting('wallet')}
                                          walletSetup={keeperSettings.wallet}/>
        },
        {
            label: 'Wallet Setup',
            caption: 'Setup wallet native and system coins',
            content: <WalletBalances walletBalances={keeperSettings.walletSetup} system={keeperSettings.system}
                                     refreshWalletBalances={getWalletBalances}/>
        },
        {
            label: 'Keeper Options',
            caption: 'Setup the keeper options',
            content: <KeeperOptions options={keeperSettings.options} handleKeeperOptions={handleKeeperOption}/>
        },
    ];
